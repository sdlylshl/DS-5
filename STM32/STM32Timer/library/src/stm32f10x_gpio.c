  ...
  if (((u32)GPIO_InitStruct->GPIO_Pin & ((u32)0x00FF)) != 0x00)
  {
    ...
    for (pinpos = 0x00; pinpos < 0x08; pinpos++)
    {
      ...
      if (currentpin == pos)
      {
        ...
        {
          GPIOx->BRR = (((u32)0x01) << pinpos);//BRR和BSRR只在写1起作用，写0不起作用
        }
...
u8 GPIO_ReadOutputDataBit(GPIO_TypeDef* GPIOx, u16 GPIO_Pin)
{
  ...
  /* Check the parameters */
  assert(IS_GET_GPIO_PIN(GPIO_Pin)); 		  //每次只读1位
...
{
  /* Check the parameters */
  assert(IS_GPIO_PIN(GPIO_Pin));  //可同时置位多个I/O口
  GPIOx->BSRR = GPIO_Pin;
}
...
{
  /* Check the parameters */
  assert(IS_GPIO_PIN(GPIO_Pin)); //可同时复位多个I/O口
  GPIOx->BRR = GPIO_Pin;
}
...
{
  /* Check the parameters */
  assert(IS_GET_GPIO_PIN(GPIO_Pin));//该函数每次只能写一个位
...
void GPIO_PinLockConfig(GPIO_TypeDef* GPIOx, u16 GPIO_Pin)
{
  ...
  /* Check the parameters */
  assert(IS_GPIO_PIN(GPIO_Pin)); //可同时锁定多位
