...
void RCC_HSEConfig(u32 RCC_HSE)
{
  ...
  /*配置HSE(RCC_HSE_OFF已经被以上代码段所包含（执行过了已经）)*/
  switch(RCC_HSE)
  {
    case RCC_HSE_ON:
      /* Set HSEON bit */
...
* Input          : - RCC_AHBPeriph: specifies the AHB peripheral to gates(门控) its clock.
*                    This parameter can be any combination of the following values:
*                       - RCC_AHBPeriph_DMA
*                       - RCC_AHBPeriph_SRAM
*                       - RCC_AHBPeriph_FLITF
...
* Description    : Clears the RCC interrupt pending bits.
