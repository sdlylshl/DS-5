...
#define CR1_CounterMode_Mask        ((u16)0x038F)//勘误由0x039F改为0x038F
...
#define CCMR_CC13S_Mask             ((u16)0x7F7C)  //为何不是0XFFFC?
...
*                  the specified parameters in the TIM_TimeBaseInitStruct.
                   该函数配置了ARR的值；选定的输入时钟到定时器时钟（CK_CNT）的分频
				   系数；采样频率（Tdts）跟CK_CNT的分频系数；计数模式
* Input          : - TIMx: where x can be 2, 3 or 4 to select the TIM peripheral.
...
void TIM_OCInit(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
      ...
      (TIM_OCInitStruct->TIM_Channel == (u16)TIM_Channel_2))
  {
    ...
    if (TIM_OCInitStruct->TIM_Channel == TIM_Channel_1)
    {
	  ...
	  //设置输出模式 ，为何没有强制输出的两种模式？
	  //原来有1个单独函数（TIM_ForcedOC1Config）配置强制输出模式
...
void TIM_TimeBaseStructInit(TIM_TimeBaseInitTypeDef* TIM_TimeBaseInitStruct)
{
  ...
  TIM_TimeBaseInitStruct->TIM_Period = TIM_Period_Reset_Mask; //ARR=0
  TIM_TimeBaseInitStruct->TIM_Prescaler = TIM_Prescaler_Reset_Mask;//PRE=0
  TIM_TimeBaseInitStruct->TIM_ClockDivision = TIM_CKD_DIV1;//Tdts==Tck_int
  TIM_TimeBaseInitStruct->TIM_CounterMode = TIM_CounterMode_Up;//边缘对齐向上计数
}
...
{
  /* Set the default configuration */
  TIM_OCInitStruct->TIM_OCMode = TIM_OCMode_Timing;	//冻结模式
  TIM_OCInitStruct->TIM_Channel = TIM_Channel_1;  //默认通道1
  TIM_OCInitStruct->TIM_Pulse = TIM_Pulse_Reset_Mask;//CRR=0
...
{
  /* Set the default configuration */
  TIM_ICInitStruct->TIM_ICMode = TIM_ICMode_ICAP;//输入捕获模式
  TIM_ICInitStruct->TIM_Channel = TIM_Channel_1; //默认通道1
  TIM_ICInitStruct->TIM_ICPolarity = TIM_ICPolarity_Rising;//输入不反相
  TIM_ICInitStruct->TIM_ICSelection = TIM_ICSelection_DirectTI;//选择对应的引脚，如IC1选择IT1
  TIM_ICInitStruct->TIM_ICPrescaler = TIM_ICPSC_DIV1;//不分频
  TIM_ICInitStruct->TIM_ICFilter = TIM_ICFilter_Mask;//无滤波
}
...
* Description    : Configures the TIMx interrnal Clock，关闭从模式（注意：也不是进入主模式）
                   时钟由内部时钟CK_INT驱动
* Input          : - TIMx: where x can be 2, 3 or 4 to select the TIM peripheral.
* Output         : None
* Return         : None
...
* Description    : Configures the TIMx Trigger as External Clock
                   配置外部时钟模式1，同时限定触发源为TI1FP1、TI12FP2、TI1F_ED
* Input          : - TIMx: where x can be 2, 3 or 4 to select the TIM peripheral.
...
* Output         : None
* Return         : None
*疑问：为何使能捕获（在TI1_Config中）
*注意：TIM_TS_TI1F_ED 和TIM_TS_TI1FP1在通道选择上是一样的，在触发源选择上不同而已
*******************************************************************************/
...
/*******************************************************************************
* Function Name  : TIM_SelectInputTrigger选择触发源
...
* Output         : None
* Return         : None
* 疑问：此函数与TIM_TimeBaseInit函数中部分功能重合，有存在必要？
*******************************************************************************/
void TIM_PrescalerConfig(TIM_TypeDef* TIMx, u16 Prescaler, u16 TIM_PSCReloadMode)
{
  ...
  //此分频器虽然有预装载功能，却不像ARR和CRR那样有相关的位控制立即装载或更新事件装载
  //要实现立即装载必须手动产生更新事件
  ...
  {
    TIMx->EGR &= ~TIM_EventSource_Update;//勘误 加了个~
  }
}
...
* Description    : Enables or Disables the TIMx Update event.
                   使能或禁止更新事件（UEV），禁止更新事件时，设置UG位或从模式控制器
				   发出复位信号时影子寄存器值保持不变，计数器和分频器重新归零
* Input          : - TIMx: where x can be 2, 3 or 4 to select the TIM peripheral.
*                  - Newstate: new state of the TIMx peripheral Preload register
*                    This parameter can be: ENABLE or DISABLE.
* Output         : None
...
*                    This parameter can be one of the following values:
*                       - TIM_UpdateSource_Regular:只有计数器溢出产生UEV
*                       - TIM_UpdateSource_Global：计数器溢出、设置UG位、从模式控制器更新都产生UEV
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : Enables or disables the TIMx Hall sensor interface.
                   使能或禁止霍尔传感器接口
* Input          : - TIMx: where x can be 2, 3 or 4 to select the TIM peripheral.
...
*                       - TIM_TRGOSource_Update //更新事件（一般为定时器溢出，取决于URS设置）
*                       - TIM_TRGOSource_OC1
...
* Output         : None
* Return         : None
* 疑问           ：此函数跟TIM_TimeBaseInit函数部分功能重合  
*******************************************************************************/
...
/*******************************************************************************
* Function Name  : PWMInput_Config 配置为PWM输入模式
...
* Description    : Configure the TI1 as Input. 配置通道1为输入捕获模式，配置输入选择、
                                               输入滤波器和输入极性3个参数并使能捕获
* Input          : - TIMx: where x can be 2, 3 or 4 to select the TIM peripheral.
