...
typedef struct
{
  u16 TIM_Period;          /* Period value（ARR值） */
  u16 TIM_Prescaler;       /* Prescaler value 输入时钟到定时器时钟（CK_CNT）的分频系数*/				   
  u16 TIM_ClockDivision;   /* Timer clock division 采样频率（Tdts）跟CK_CNT的分频系数*/
  u16 TIM_CounterMode;     /* Timer Counter mode计数模式 */
} TIM_TimeBaseInitTypeDef;

/* TIM Output Compare Init structure definition */
typedef struct
{
  u16 TIM_OCMode;          /* Timer Output Compare Mode 输出模式（见模式定义）*/
  u16 TIM_Channel;          /* Timer Channel 输出通道（1--4）*/
  u16 TIM_Pulse;           /* PWM or OC Channel pulse length脉宽（CCRx值） */
  u16 TIM_OCPolarity;       /* PWM, OCM or OPM Channel polarity 输出极性*/
} TIM_OCInitTypeDef;
...
#define TIM_OCMode_Timing                 ((u16)0x0000)	//冻结模式
#define TIM_OCMode_Active                 ((u16)0x0010)	//TIMx_CNT==TIMx_CCRx时，OCx_REF为1
#define TIM_OCMode_Inactive               ((u16)0x0020)	//TIMx_CNT==TIMx_CCRx时，OCx_REF为0
#define TIM_OCMode_Toggle                 ((u16)0x0030)	//TIMx_CNT==TIMx_CCRx时，OCx_REF翻转
#define TIM_OCMode_PWM1                   ((u16)0x0060)	//PWM模式1
#define TIM_OCMode_PWM2                   ((u16)0x0070)	//PWM模式2
...
#define TIM_ExtTRGPolarity_Inverted       ((u16)0x8000)//反相
#define TIM_ExtTRGPolarity_NonInverted    ((u16)0x0000)//不反相
