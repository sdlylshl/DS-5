...
// 串口参数
#define  UARTx_BaudRate				UART5_BaudRate			/* 波特率 */
...
#define  UARTx_Config				UART5_Config			/* 配置函数名 */
#define  UARTx_SendBytes			UART5_SendBytes			/* 发送函数名 */
#define  UARTx_RcvdSize				UART5_RcvdSize			/* 接收字节数 */
#define  UARTx_ReadBytes			UART5_ReadBytes			/* 接收函数名 */
// 引脚及内核相关定义
#define  UARTx						UART5			/* USART_TypeDef定义的UART基地址 */
...
#define  UARTx_IRQHandler			UART5_IRQHandler		/* UARTx中断处理函数名称, 注意为UART/USART */
#define  UARTx_GPIO					GPIOC					/* UARTx引脚所使用的端口 */
#define  UARTx_GPIO_RCC				RCC_APB2Periph_GPIOC	/* GPIOA RCC 名称定义 */
#define  UARTx_GPIO_Pin_Tx			GPIO_Pin_12				/* 根据硬件定义 */
#define  UARTx_GPIO_Pin_Rx			GPIO_Pin_2				/* 根据硬件定义 */
// DMA定义
#define  UARTx_DMA_EN				0						/* 1:启用DMA,0:不使用DMA,1:直接使用UART中断收发数据 */
#define  UARTx_DMA_RX_EN			0						/* 1:启用DMA接收中断,0:使用UART中断接收数据,一般不使用DMA接收数据 */
#define  UARTx_RCC_DMA				RCC_AHBPeriph_DMA2		/* 定义则开户DMA */
#define  UARTx_DMA					DMA2					/* 定义DMA寄存器 */
#define  UARTx_DMA_Rx_Channel		DMA2_Channel3			/* UART.RX对应的DMA通道 */
...
#define  UARTx_Rx_TIM_Start()		UART1T4_Rx_TIM_CC_Start(5, 5)	/* 开启定时器通道 */
#define  UARTx_Rx_isTimeOut()		UART1T4_Rx_isTimeOut(5)			/* 关闭定时器通道 */
#else
...
#else
#define  UARTx_ENTER_CRITICAL_TX()	UARTx_ENTER_CRITICALx_TX(5)	/* CLRBITs(UARTx->CR1, USART_FLAG_TC)	, 关中断 */
#define  UARTx_EXIT_CRITICAL_TX()	UARTx_EXIT_CRITICALx_TX(5)	/* SETBITs(UARTx->CR1, USART_FLAG_TC)	 开中断 */
#endif
#define  UARTx_ENTER_CRITICAL_RX()	UARTx_ENTER_CRITICALx_RX(5)	/* CLRBITs(UARTx->CR1, USART_FLAG_RXNE), 关中断 */
#define  UARTx_EXIT_CRITICAL_RX()	UARTx_EXIT_CRITICALx_RX(5)	/* SETBITs(UARTx->CR1, USART_FLAG_RXNE), 开中断 */
