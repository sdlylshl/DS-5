...
// 串口参数
#define  UARTx_BaudRate				UART3_BaudRate			/* 波特率 */
...
#define  UARTx_Config				UART3_Config			/* 配置函数名 */
#define  UARTx_SendBytes			UART3_SendBytes			/* 发送函数名 */
#define  UARTx_RcvdSize				UART3_RcvdSize			/* 接收字节数 */
#define  UARTx_ReadBytes			UART3_ReadBytes			/* 接收函数名 */
#define  UARTx_GPIO_Remap			GPIO_FullRemap_USART3	/* IO重映射 */
// 引脚及内核相关定义
#define  UARTx						USART3			/* USART_TypeDef定义的UART基地址 */
...
#define  UARTx_IRQHandler			USART3_IRQHandler		/* UARTx中断处理函数名称, 注意为UART/USART */
#define  UARTx_GPIO					GPIOD					/* UARTx引脚所使用的端口 */
#define  UARTx_GPIO_RCC				RCC_APB2Periph_GPIOD	/* GPIOA RCC 名称定义 */
#define  UARTx_GPIO_Pin_Tx			GPIO_Pin_8				/* 根据硬件定义 */
#define  UARTx_GPIO_Pin_Rx			GPIO_Pin_9				/* 根据硬件定义 */
// DMA定义
...
#define  UARTx_DMA_Rx_Channel		DMA1_Channel3			/* UART.RX对应的DMA通道 */
#define  UARTx_DMA_Rx_NVICPrioGroup	NVIC_PriorityGroup_3	/* 中断向量优先级组 */
#define  UARTx_DMA_Rx_IRQn			DMA1_Channel3_IRQn		/* 中断向量号 */
...
#define  UARTx_DMA_Rx_IF_TC			DMA1_FLAG_TC3			/* 定义通道中断标志 */
#define  UARTx_DMA_Rx_IF_GL			DMA1_FLAG_GL3			/* 全局中断标志 */
#define  UARTx_DMA_Rx_IF_HT			DMA1_FLAG_HT3			/* 半传输完成中断标志 */
void 	 DMA1_Channel3_IRQHandler(void);
#define  UARTx_DMA_Rx_IRQHandler	DMA1_Channel3_IRQHandler	/* Rx中断处理函数名,注意此名称定义无效必须更改函数正式说明 */
// 接收超时定时器
...
#define  UARTx_Rx_TIM_Start()		UART1T4_Rx_TIM_CC_Start(3, 3)	/* 开启定时器通道 */
#define  UARTx_Rx_isTimeOut()		UART1T4_Rx_isTimeOut(3)			/* 关闭定时器通道 */
#else
...
#define  UARTx_DMA_Tx_Channel		DMA1_Channel2			/* DMA通道 */
#define  UARTx_DMA_Tx_NVICPrioGroup	NVIC_PriorityGroup_3	/* 中断向量优先级组 */
#define  UARTx_DMA_Tx_IRQn			DMA1_Channel2_IRQn		/* 中断向量号 */
...
#define  UARTx_DMA_Tx_IF_TC			DMA1_FLAG_TC2			/* 定义通道中断标志 */
#define  UARTx_DMA_Tx_IF_TE			DMA1_FLAG_TE2			/* 传输错误中断标志 */
#define  UARTx_DMA_Tx_IF_GL			DMA1_FLAG_GL2			/* 全局中断标志 */
#define  UARTx_DMA_Tx_Number		0x0102					/* 高字节为DMA号1/2, 低字节为通道号 */
void 	 DMA1_Channel2_IRQHandler(void);
#define  UARTx_DMA_Tx_IRQHandler	DMA1_Channel2_IRQHandler	/* Tx中断处理函数名 */
// 中断管理设置
#if UARTx_DMA_EN > 0
...
#else
#define  UARTx_ENTER_CRITICAL_TX()	UARTx_ENTER_CRITICALx_TX(3)	/* CLRBITs(UARTx->CR1, USART_FLAG_TC)	, 关中断 */
#define  UARTx_EXIT_CRITICAL_TX()	UARTx_EXIT_CRITICALx_TX(3)	/* SETBITs(UARTx->CR1, USART_FLAG_TC)	 开中断 */
#endif
#define  UARTx_ENTER_CRITICAL_RX()	UARTx_ENTER_CRITICALx_RX(3)	/* CLRBITs(UARTx->CR1, USART_FLAG_RXNE), 关中断 */
#define  UARTx_EXIT_CRITICAL_RX()	UARTx_EXIT_CRITICALx_RX(3)	/* SETBITs(UARTx->CR1, USART_FLAG_RXNE), 开中断 */
