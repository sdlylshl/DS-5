...
// 串口参数
#define  UARTx_BaudRate				UART1_BaudRate			/* 波特率 */
...
#define  UARTx_Config				UART1_Config			/* 配置函数名 */
#define  UARTx_SendBytes			UART1_SendBytes			/* 发送函数名 */
#define  UARTx_RcvdSize				UART1_RcvdSize			/* 接收字节数 */
#define  UARTx_ReadBytes			UART1_ReadBytes			/* 接收函数名 */
// 引脚及内核相关定义
#define  UARTx						USART1			/* USART_TypeDef定义的UART基地址 */
...
#define  UARTx_IRQHandler			USART1_IRQHandler		/* UARTx中断处理函数名称, 注意为UART/USART */
#define  UARTx_GPIO					GPIOA					/* UARTx引脚所使用的端口 */
#define  UARTx_GPIO_RCC				RCC_APB2Periph_GPIOA	/* GPIOA RCC 名称定义 */
#define  UARTx_GPIO_Pin_Tx			GPIO_Pin_9				/* 根据硬件定义 */
#define  UARTx_GPIO_Pin_Rx			GPIO_Pin_10				/* 根据硬件定义 */
// DMA定义
...
#define  UARTx_DMA_Rx_Channel		DMA1_Channel5			/* UART.RX对应的DMA通道 */
#define  UARTx_DMA_Rx_NVICPrioGroup	NVIC_PriorityGroup_3	/* 中断向量优先级组 */
#define  UARTx_DMA_Rx_IRQn			DMA1_Channel5_IRQn		/* 中断向量号 */
...
#define  UARTx_DMA_Rx_IF_TC			DMA1_FLAG_TC5			/* 定义通道中断标志 */
#define  UARTx_DMA_Rx_IF_GL			DMA1_FLAG_GL5			/* 全局中断标志 */
#define  UARTx_DMA_Rx_IF_HT			DMA1_FLAG_HT5			/* 半传输完成中断标志 */
void 	 DMA1_Channel5_IRQHandler(void);
#define  UARTx_DMA_Rx_IRQHandler	DMA1_Channel5_IRQHandler	/* Rx中断处理函数名,注意此名称定义无效必须更改函数正式说明 */
// 接收超时定时器
...
#define  UARTx_Rx_TIM_Start()		UART1T4_Rx_TIM_CC_Start(1, 1)	/* 开启定时器通道 */
#define  UARTx_Rx_isTimeOut()		UART1T4_Rx_isTimeOut(1)			/* 关闭定时器通道 */
#else
...
#define  UARTx_DMA_Tx_Channel		DMA1_Channel4			/* DMA通道 */
#define  UARTx_DMA_Tx_NVICPrioGroup	NVIC_PriorityGroup_3	/* 中断向量优先级组 */
#define  UARTx_DMA_Tx_IRQn			DMA1_Channel4_IRQn		/* 中断向量号 */
...
#define  UARTx_DMA_Tx_IF_TC			DMA1_FLAG_TC4			/* 定义通道中断标志 */
#define  UARTx_DMA_Tx_IF_TE			DMA1_FLAG_TE4			/* 传输错误中断标志 */
#define  UARTx_DMA_Tx_IF_GL			DMA1_FLAG_GL4			/* 全局中断标志 */
#define  UARTx_DMA_Tx_Number		0x0104					/* 高字节为DMA号1/2, 低字节为通道号 */
void 	 DMA1_Channel4_IRQHandler(void);
#define  UARTx_DMA_Tx_IRQHandler	DMA1_Channel4_IRQHandler	/* Tx中断处理函数名 */
// 中断管理设置
#if UARTx_DMA_EN > 0
...
#else
#define  UARTx_ENTER_CRITICAL_TX()	UARTx_ENTER_CRITICALx_TX(1)	/* CLRBITs(UARTx->CR1, USART_FLAG_TC)	, 关中断 */
#define  UARTx_EXIT_CRITICAL_TX()	UARTx_EXIT_CRITICALx_TX(1)	/* SETBITs(UARTx->CR1, USART_FLAG_TC)	 开中断 */
#endif
#define  UARTx_ENTER_CRITICAL_RX()	UARTx_ENTER_CRITICALx_RX(1)	/* CLRBITs(UARTx->CR1, USART_FLAG_RXNE), 关中断 */
#define  UARTx_EXIT_CRITICAL_RX()	UARTx_EXIT_CRITICALx_RX(1)	/* SETBITs(UARTx->CR1, USART_FLAG_RXNE), 开中断 */
