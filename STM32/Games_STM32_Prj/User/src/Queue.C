...
** 描　述: 修改返回值: uint8->uint16, 删除传入参数Ret
**------------------------------------------------------------------------------------------------------
********************************************************************************************************/
uint16 QueueRead(/*QUEUE_DATA_TYPE *Ret,*/ void *Buf)
{
	register uint8 err;
...
uint8 QueueWriteBytes(void *Buf, QUEUE_DATA_TYPE *pSrc, uint16 NData)
{
	register QUEUE_DATA_TYPE *pQueIn;
	register QUEUE_DATA_TYPE *pQueEnd;
	register QUEUE_DATA_TYPE *pSrcData;
	register uint16 nDataSize;
...
#define EN_QUEUE_OUT_N    0
        ...
        void QueueOutN(void *Buf, uint32 N)
{
    register T_DataQueue *Queue;
    register uint32 N0;
    ...
    {                                                               /* 有效         */
        ...
        //Queue->Out = (void *)((uint32)Queue->Buf + 				// Del by Xsky 2011-07-29 11:50
        //			  	(uint32)(Queue->Out + N)-(uint32)Queue->End);
