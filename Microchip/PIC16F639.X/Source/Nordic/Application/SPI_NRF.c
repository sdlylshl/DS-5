...
//return (GPIO_ReadInputDataBit(NRF_IRQ_PORT, NRF_IRQ_PIN)); //低电平产生中断
	...
	//return SPI2_READ_IRQ();		//SPI2
	return SPI_IRQ_READ();			//模拟SPI 2
}

static __inline uint8_t SPI_NRF_RW(uint8_t dat){
	//return SPI2_SendByte(dat);
	return SPI_ReadWrite(dat);
}
...
uint8_t NRF_Tx_Dat(uint8_t *txbuf)
{
   ...
   SPI_NRF_WriteBuf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);
	...
	SPI_NRF_WriteReg(FLUSH_TX,NRF_NOP);    //清除TX FIFO寄存器
...
uint8_t NRF_Rx_Dat(uint8_t *rxbuf)
{
	...
	if(state&RX_DR)                                 //接收到数据
	{
	  SPI_NRF_ReadBuf(RD_RX_PLOAD,rxbuf,RX_PLOAD_WIDTH);//读取数据
	     SPI_NRF_WriteReg(FLUSH_RX,NRF_NOP);          //清除RX FIFO寄存器
	  return RX_DR; 
	}
	else    
