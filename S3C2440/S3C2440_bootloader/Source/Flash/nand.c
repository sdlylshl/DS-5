...
struct Partition *NandSelPart(char *info)
{
    ...
    printf("\nPlease select which region to %s : Esc to abort   SPACE to download\n", info);
...
char CheckApp(){
		...
		page_cnt = nf_part->size>>11; //每页有2^11 2048字节，求出所占用的页数。
		//页写
...
static void LoadRunApp()
{
	...
	IIC_Read(0x18,&updateflg);
	if(updateflg){
    nf_part = NandSelPart_2("kernel");
  	printf("Select kernelApp\n");
    }else{
    nf_part = NandSelPart_2("MyApp");
  	printf("Select MyApp\n");
    }
    ...
    //size = nf_part->size;
    /*
    size = (boot_params.appsize.val & (~0x7FF)) + 0x800;
    if ((size < 1024 * 100) || (size > nf_part->size))
        size = (1024 + 512) * 1024;
    printf("Application size is %d MB\n\n", (size));
    if ((U32)(size / 1024) < 1024)
        printf("Application size is %4.2f KB\n\n", (float)(size / 1024));
    else
        printf("Application size is %4.2f MB\n\n", (float)(size / 1024 / 1024.));
    ...
    */
    ...
    ram_addr = boot_params.AppRun_addr.val;
	 printf("boot_params.AppRun_addr.val is %x\n\n", boot_params.AppRun_addr.val);
    for (i = 0; size > 0; )
    {
        if (!(i & 0x3f))
        {
