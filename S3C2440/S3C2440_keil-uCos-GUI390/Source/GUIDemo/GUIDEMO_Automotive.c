...
0,	/* No transparency */ & _ColorsScaleR140[0]
};
...
0, /* No transparency */ & _ColorsScaleR80[0]
};
...
static void _AutomotiveDemo(void)
{
	...
	for (Cnt = 0;
		 ((tDiff = GUI_GetTime() - t0) < 30000) && !GUIDEMO_CheckCancel();
		 Cnt++)
	{
		...
		if (Angle_1_Old != Param[1].Angle)
		{
			...
			t1 = GUI_GetTime();
			GUI_RotatePolygon(Param[1].aPoints,
							  _aNeedle_1,
							  countof(_aNeedle_1),
							  Param[1].Angle);
			GUI_MEMDEV_DrawAuto(&AutoDev[1],
								&Param[1].AutoDevInfo,
								_Draw_1,
								&Param[1]);
...
static void _AutomotiveDemo(void)
{
	...
	/* Show needle for a fixed time */
	for (Cnt = 0;
		 ((tDiff = GUI_GetTime() - t0) < 16000) && !GUIDEMO_CheckCancel();
		 Cnt++)
	{
		/* Get values to display and calculate polygon for needle 2 */
