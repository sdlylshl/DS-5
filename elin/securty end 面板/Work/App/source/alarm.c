...
u8 get_alarm_out(u8 out)
{
	...
	if(out  < 4)
		...
		;//ret = GPIO_ReadInputDataBit(GPIOB, IO_OUTPUT_4<<out);	
	return ret;
}
...
{	
	...
	{
		if(val)
			;//GPIO_WriteBit(GPIOB, IO_OUTPUT_4<<out, Bit_SET);
		else
			;//GPIO_WriteBit(GPIOB, IO_OUTPUT_4<<out, Bit_RESET);
	}
...
u8 do_arm_link(u16 alarm_out, u16 alarm_close)
{
	int i;
...
u8 do_disarm_link(u16 alarm_out, u16 alarm_close)
{
	u8 i;
