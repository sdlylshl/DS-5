...
u8 ds1302_set_time(u8* src)
{
	...
	dst[0] = ((src[6]/10)<<4) + (src[6]%10);
	dst[1] = ((src[5]/10)<<4) + (src[5]%10);

	dst[2] = ((src[4]/10)<<4) + (src[4]%10);
	dst[3] = ((src[3]/10)<<4) + (src[3]%10);

	dst[4] = ((src[2]/10)<<4) + (src[2]%10);
	dst[5] = src[1];

	dst[6] = ((src[0]/10)<<4) + (src[0]%10);

	printf("sec:%d, min: %d, hour:%d, day: %d, mon:%d, week: %d, year:%d\n",
		src[6], src[5], src[4], src[3], src[2], src[1], src[0]);
