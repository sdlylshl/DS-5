...
u8 save_arm_link_to_eeprom(u32 arm)
{
	u8 i;
	u8 ch[4];
	...
	{	
		ch[i] = (arm>>(8*(3-i)))&0xff;
		printf("write ch[%d]: 0x%x\n", i, ch[i]);
		AT24CXX_WriteOneByte(EEPROM_ARM_LINK_OFFSET+i, ch[i]);
	
	}
	return OK;
}

u8 get_arm_link_from_eeprom(u32* arm)
{
	u8 i;
	u8 ch[4];
	...
	for(i=0; i<4; i++)
	{	
		ch[i] = AT24CXX_ReadOneByte(EEPROM_ARM_LINK_OFFSET+i);
		//printf("get ch[%d]: 0x%x\n", i, ch[i]);
		*arm += ch[i] << (8*(3-i));
	}
...
u8 save_disarm_link_to_eeprom(u32 arm)
{
	u8 i;
	u8 ch[4];
	...
	{	
		ch[i] = (arm>>(8*(3-i)))&0xff;
		printf("write ch[%d]: 0x%x\n", i, ch[i]);
		AT24CXX_WriteOneByte(EEPROM_DISARM_LINK_OFFSET+i, ch[i]);
	
	}
	return OK;
}

u8 get_disarm_link_from_eeprom(u32* arm)
{
	u8 i;
	u8 ch[4];
	...
	for(i=0; i<4; i++)
	{	
		ch[i] = AT24CXX_ReadOneByte(EEPROM_DISARM_LINK_OFFSET+i);
		printf("get ch[%d]: 0x%x\n", i, ch[i]);
		*arm += ch[i] << (8*(3-i));
	}
...
{	
	...
	{	
		ch[j] = (in>>(8*(3-j)))&0xff;
		AT24CXX_WriteOneByte(EEPROM_LINK_IN_OFFSET+i*12+j, ch[j]);
	...
	{	
		ch[j] = (out>>(8*(1-j)))&0xff;
		AT24CXX_WriteOneByte(EEPROM_LINK_OUT_OFFSET+i*12+j, ch[j]);
...
u8 get_link_from_eeprom(u8 i, u32* in, u16* out, u8* xor)
{
	...
	for(j=0; j<4; j++)
	{	
		ch[j] = AT24CXX_ReadOneByte(EEPROM_LINK_IN_OFFSET+i*12+j);
		*in += ch[j] << (8*(3-j));
	}

	for(j=0; j<2; j++)
	{	
		ch[j] = AT24CXX_ReadOneByte(EEPROM_LINK_OUT_OFFSET+i*12+j);
		*out += ch[j] << (8*(1-j));
	}
...
u8 save_cell_num_to_eeprom(u8 i, u8* src)
{
	...
	for(j=0; j<11; j++)
	{
		AT24CXX_WriteOneByte(EEPROM_CELL_NUM1_OFFSET+i*16+j, src[j]);
		printf("addr: %c",  src[j]);
	}
	return OK;
}
...
u8 get_cell_num_from_eeprom(u8 i, u8* src)
{
	...
	for(j=0; j<11; j++)
	{	
		src[j] = AT24CXX_ReadOneByte(EEPROM_CELL_NUM1_OFFSET+i*16+j);
	}
...
{	
	...
	for(j=0; j<11; j++)
	{
		AT24CXX_WriteOneByte(EEPROM_TEXT_NUM1_OFFSET+i*16+j, src[j]);
		printf("addr: %d", src[j]);
	}
	return OK;
...
u8 get_text_num_from_eeprom(u8 i, u8* src)
{
	...
	for(j=0; j<11; j++)
	{	
		src[j] = AT24CXX_ReadOneByte(EEPROM_TEXT_NUM1_OFFSET+i*16+j);
	}
...
u8 save_text_content_to_eeprom(u8 i, u8 len, u8* src)
{
	...
	for(j=0; j<len; j++)
	{
		AT24CXX_WriteOneByte(EEPROM_TEXT_CONTENT1_OFFSET+i*64+j, src[j]);
		printf("addr: %d", src[j]);
	}
	return OK;
...
u8 get_text_content_from_eeprom(u8 i, u8 len, u8* src)
{
	...
	for(j=0; j<len; j++)
	{	
		src[j] = AT24CXX_ReadOneByte(EEPROM_TEXT_CONTENT1_OFFSET+i*64+j);
	}

	return OK;
}

u8 save_para(sys_para* sys)
{
