...
#define		STARTADDR 0x08020000

#define 	UPGRADE_OFFSET  0x08050000 
#define 	UPGRADE_FLAG	0x0807F800
...
{	
	...
	FLASH_UnlockBank1();
	FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR); 
    FLASHStatus = FLASH_ErasePage(UPGRADE_OFFSET+flash_page_index*MAX_FLASH_PAGE_CAPICITY);

	if (a<=MAX_FLASH_PAGE_CAPICITY/4)
	{
		if(FLASHStatus == FLASH_COMPLETE)
		{	
			for(i=0; i<a; i++)
			{
				if(FLASHStatus == FLASH_COMPLETE)
					FLASHStatus = FLASH_ProgramWord((UPGRADE_OFFSET+flash_page_index*MAX_FLASH_PAGE_CAPICITY) + i*4, *(u32*)(upgrade_buf+4*i));
				else
	...
	for(i=0; i<b; i++)
	{
		p += ((*(upgrade_buf+a*4+i)) << (i*8));
	}
	FLASHStatus = FLASH_ProgramWord((UPGRADE_OFFSET+flash_page_index*MAX_FLASH_PAGE_CAPICITY) + a*4, p);
...
int do_write_upgrade_flag(u32 len)
{
	FLASH_UnlockBank1();
	FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR); 
    FLASHStatus = FLASH_ErasePage(UPGRADE_FLAG);

	if(FLASHStatus == FLASH_COMPLETE)
	{
		FLASHStatus = FLASH_ProgramWord(UPGRADE_FLAG, 0x55aaaa55);  
		FLASHStatus = FLASH_ProgramWord(UPGRADE_FLAG+4, len);
	}
	FLASH_LockBank1();
	return OK;
}
